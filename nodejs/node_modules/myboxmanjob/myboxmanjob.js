...
var job = function(credentials,debug) {
  ...
  this.confirmPickup = function(confirmationCode,callback) {
    var self = this;
    if(self._id=="") {
      console.log('Job should be initiated before, use loadJobFromJobId()');
      return false;
    }
    var waitingInterval = setInterval(function() {
      if (self._waitingCalls==0) {
  ...
  this.confirmDelivery = function(callback) {
    var self = this;
    if(self._id=="") {
      console.log('Job should be initiated before, use loadJobFromJobId()');
      return false;
    ...
    var waitingInterval = setInterval(function() {
      if (self._waitingCalls==0) {
        ...
        var params = {
         ...
         reqOptions.url='confirmDelivery';
         reqOptions.form = params;
  ...
  this.loadJobFromJobId = function(jobId,callback) {
    ...
    request(reqOptions,function(err,res,body) {
      ...
      self._loadingDateTime1 = new Date(job.loadingDate1.timeStamp);
      self._loadingDateTime2 = new Date(job.loadingDate2.timeStamp);
      self._deliveryDateTime1 = new Date(job.deliveryDate1.timeStamp);
      self._deliveryDateTime2 = new Date(job.deliveryDate2.timeStamp);
      callback(true);
    } else {
      callback(false,body);
    }
